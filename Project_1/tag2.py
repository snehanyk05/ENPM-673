...
vidcap = cv2.VideoCapture('Tag2.mp4')
success,image = vidcap.read()
count = 0
success = True
while success and count<200:
  cv2.imwrite("tag%d.jpg" % count, image)     # save frame as JPEG file
  success,image = vidcap.read()
  #print ('Read a new frame: '+ str(success))
  count += 1

img = cv2.imread('Tag0.jpg')
height,width,layers=img.shape
video=cv2.VideoWriter('video.mp4',-1,20,(width,height)) 
for d in range(0,200):
    img = cv2.imread('tag%d.jpg'%d)
    gray = cv2.imread('tag%d.jpg'%d,0)
